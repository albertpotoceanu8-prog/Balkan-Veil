...
export function PricingPage({ content, goToPage }: PricingPageProps) {
  return (
    <PageShell eyebrow={content.eyebrow} title={content.title} text={content.text}>
      <div className="operator-surface mb-10 border border-[#33270f] bg-black/45 p-5 md:p-7">
        <div className="absolute inset-0 operator-grid opacity-10" aria-hidden="true" />
        <div className="relative max-w-4xl">
          <p className="text-xs uppercase tracking-[0.32em] text-[#d4af37]">{content.scopeNote.eyebrow}</p>
          <h2 className="mt-4 font-serif text-3xl leading-tight text-[#d8c7a3] md:text-4xl">{content.scopeNote.title}</h2>
      ...
      <div className="grid gap-8 lg:grid-cols-[0.9fr_1.12fr_0.9fr] lg:items-start">
        {content.plans.map((plan, index) => (
          <motion.div key={plan.name} {...cardMotion} className={`h-full ${plan.recommended ? "lg:-mt-8" : "lg:mt-10"}`}>
            <VeilFrame label={`${content.planIndexLabel} ${String(index + 1).padStart(2, "0")}`} index={plan.name} className={`h-full ${plan.recommended ? "border-[#b99832]/50 bg-[#050302]/92" : "bg-[#050302]/86"}`}>
              <div className="flex h-full flex-col p-6 md:p-8">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="font-mono text-xs uppercase tracking-[0.3em] text-[#d4af37]">{plan.label}</p>
                    <h2 className="mt-5 font-serif text-4xl text-[#d8c7a3] md:text-5xl">{plan.name}</h2>
                  </div>
                ...
                <div className="relative mt-9 border-y border-[#241d0c] py-6">
                  <span className={plan.recommended ? "absolute right-0 top-6 h-2 w-16 bg-[#b99832]/45" : "absolute right-0 top-6 h-2 w-16 bg-[#b99832]/30"} aria-hidden="true" />
                  <div className="flex items-end gap-2">
                    <p className="font-serif text-5xl text-[#d8c7a3] md:text-6xl">{plan.promoPrice ?? plan.price}</p>
                  ...
                  {plan.promoPrice ? (
                    <p className="mt-3 text-sm leading-6 text-[#6c5e4e]">
                      <span className="text-[#f0c85a]">{plan.promoLabel}:</span> {plan.promoNote}
                    </p>
                  ) : null}
                ...
                <div className="mt-auto space-y-4 pt-7">
                  <p className="text-sm leading-6 text-[#756650]">
                    <span className="text-[#f0c85a]">{content.supportLabel}:</span> {plan.support}
                  </p>
                  <p className="text-sm leading-6 text-[#756650]">
                    <span className="text-[#f0c85a]">{content.bestForLabel}:</span> {plan.bestFor}
                  </p>
                  <Button type="button" onClick={() => goToPage("access")} className="mt-2 inline-flex min-h-12 w-full items-center justify-center border border-[#d4af37] bg-[#d4af37] px-6 py-3 font-mono text-[10px] uppercase tracking-[0.2em] text-black transition hover:bg-[#d8c7a3]">
      ...
      <motion.div {...cardMotion} className="operator-surface mt-28 border border-[#33270f] bg-black/55 p-7 md:p-12">
        <div className="absolute inset-0 operator-grid opacity-15" aria-hidden="true" />
        <p className="text-sm uppercase tracking-[0.35em] text-[#d4af37]">{content.buyoutNote.eyebrow}</p>
        <h2 className="mt-5 font-serif text-4xl leading-tight text-[#d8c7a3] md:text-6xl">{content.buyoutNote.title}</h2>
      ...
      <div className="mt-28">
        ...
        <h2 className="mt-5 max-w-4xl font-serif text-4xl leading-tight text-[#d8c7a3] md:text-6xl">{content.faqTitle}</h2>
