        ...
        {submitted && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[1001] flex items-center justify-center bg-black/80 px-6 backdrop-blur-md md:backdrop-blur-xl" role="dialog" aria-modal="true" aria-labelledby="access-success-title" aria-describedby="access-success-description">
            ...
            >
              ...
              <p id="access-success-description" className="mt-6 text-lg leading-8 text-[#786f5e]">{content.modalText}</p>
              <button type="button" onClick={() => setSubmitted(false)} className="mt-9 border border-[#b98a32]/45 bg-[#b98a32]/10 px-8 py-4 font-mono text-[10px] uppercase tracking-[0.26em] text-[#c8ad72] transition hover:bg-[#b98a32] hover:text-black">
                {content.modalButton}
              </button>
            </motion.div>
          </motion.div>
      ...
      <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
        <Card className="operator-surface border border-[#252729] bg-[#050302]/90 text-[#c8ad72] transition duration-500 hover:border-[#7d6a45]/55">
          <CardContent className="p-7 md:p-12">
            ...
            <p className="mt-6 text-lg leading-8 text-[#786f5e]">{content.panelText}</p>
            <div className="mt-10 space-y-4 text-[#787873]">
              {content.steps.map((step, index) => (
                <p key={step}>
                  <span className="text-[#d2aa55]">{String(index + 1).padStart(2, "0")}</span> {step}
                </p>
              ))}
            </div>
            <a href="mailto:contact@balkanveil.com" className="mt-10 inline-flex min-h-14 items-center justify-center border border-[#b98a32] bg-[#b98a32] px-9 py-4 font-mono text-[10px] uppercase tracking-[0.24em] text-black transition duration-500 hover:bg-[#c8ad72]">
          ...
          <form onSubmit={submitAccessRequest} className="grid gap-5" aria-describedby={hasSubmitError ? "access-form-error" : undefined} noValidate>
              ...
              <div className="mt-5 flex flex-wrap gap-3">
                {content.projectOptions.map((item, index) => (
                  <button
                    ...
                    className={`border px-4 py-2 text-sm transition ${selectedProjectIndex === index ? "border-[#b98a32]/60 bg-[#b98a32]/10 text-[#c8ad72]" : "border-[#202224] bg-black/40 text-[#786f5e] hover:border-[#7d6a45]/55 hover:text-[#c8ad72]"}`}
                  >
                    {item}
                  </button>
                ))}
              ...
              <div className="mt-5 flex flex-wrap gap-3">
                {content.budgetOptions.map((item, index) => (
                  <button
                    ...
                    className={`border px-4 py-2 text-sm transition ${selectedBudgetIndex === index ? "border-[#b98a32]/60 bg-[#b98a32]/10 text-[#c8ad72]" : "border-[#202224] bg-black/40 text-[#786f5e] hover:border-[#7d6a45]/55 hover:text-[#c8ad72]"}`}
                  >
                    {item}
                  </button>
                ))}
                ...
                <div className="mt-5 flex flex-wrap gap-3">
                  {content.qualification.interestedOptions.map((item, index) => (
                    <button
                      ...
                      className={`border px-4 py-2 text-sm transition ${selectedInterestIndex === index ? "border-[#b98a32]/60 bg-[#b98a32]/10 text-[#c8ad72]" : "border-[#202224] bg-black/40 text-[#786f5e] hover:border-[#7d6a45]/55 hover:text-[#c8ad72]"}`}
                    >
                      {item}
                    </button>
                  ))}
                ...
                <div className="mt-5 flex flex-wrap gap-3">
                  {content.qualification.startOptions.map((item, index) => (
                    <button
                      ...
                      className={`border px-4 py-2 text-sm transition ${selectedStartIndex === index ? "border-[#b98a32]/60 bg-[#b98a32]/10 text-[#c8ad72]" : "border-[#202224] bg-black/40 text-[#786f5e] hover:border-[#7d6a45]/55 hover:text-[#c8ad72]"}`}
                    >
                      {item}
                    </button>
                  ))}
            ...
            <div className="border border-[#252729] bg-black/35 p-5" aria-live="polite">
              ...
              <p className="mt-1 text-[#aaa59a]">
                <span className="text-[#d2aa55]">{content.form.budget}:</span> {selectedBudget}
              </p>
              <p className="mt-1 text-[#aaa59a]">
                <span className="text-[#d2aa55]">{content.qualification.interestedLabel}:</span> {selectedInterest}
              </p>
              <p className="mt-1 text-[#aaa59a]">
                <span className="text-[#d2aa55]">{content.qualification.startLabel}:</span> {selectedStart}
              </p>
            </div>

            <p id="access-form-error" className="min-h-5 text-sm text-red-300" role="alert" aria-live="polite">{submitError}</p>

            <Button type="submit" disabled={submitting} aria-busy={submitting} className="inline-flex min-h-14 items-center justify-center border border-[#b98a32] bg-[#b98a32] px-9 py-4 font-mono text-[10px] uppercase tracking-[0.24em] text-black transition duration-500 hover:bg-[#c8ad72] disabled:cursor-not-allowed disabled:opacity-70">
            ...
            <motion.div key={item.name} {...cardMotion}>
              <Card className={cardClass}>
                <CardContent className="p-9">
                  ...
                  <h3 className="mt-6 font-serif text-4xl text-[#c8ad72]">{item.name}</h3>
        ...
        <div className="mt-8 grid gap-6 md:grid-cols-2">
          {content.faq.map((item) => (
            <motion.div key={item.q} {...cardMotion} className={panelClass + " operator-surface rounded-none p-8"}>
              ...
              <p className="mt-5 text-lg leading-8 text-[#786f5e]">{item.a}</p>
            </motion.div>
          ))}
        </div>
      </div>
