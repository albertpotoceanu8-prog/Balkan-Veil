...
export function HomePage({ content, goToPage, cinematic, introDone }: HomePageProps) {
  return (
    <>
      <section className="relative z-10 mx-auto flex min-h-[calc(100svh-96px)] max-w-[1500px] flex-col justify-center overflow-hidden px-5 py-10 md:min-h-[calc(100vh-160px)] md:px-8 md:py-20">
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9 }} className="relative grid gap-10 lg:grid-cols-[1.12fr_0.88fr] lg:items-center">
          <div>
            <div className="mb-7 inline-flex max-w-full items-center gap-2 border border-amber-300/20 bg-amber-300/5 px-4 py-3 text-[11px] uppercase tracking-[0.2em] text-amber-200 shadow-[inset_0_1px_0_rgba(255,255,255,0.035)] sm:px-5 sm:text-sm sm:tracking-[0.3em]">
              <Lock className="h-4 w-4 shrink-0" aria-hidden="true" /> <span>{content.badge}</span>
            </div>
            <h1 className="max-w-6xl font-serif text-[clamp(3rem,15vw,5.75rem)] leading-[0.95] text-stone-100 sm:text-6xl md:text-8xl md:leading-[0.9] lg:text-[8.8rem]">
              {cinematic ? <DecodeText text={content.hero} canStart={introDone} /> : content.hero}
            </h1>
            <p className="mt-7 max-w-4xl text-base leading-8 text-stone-400 md:mt-10 md:text-2xl md:leading-10">{content.text}</p>

            <div className="mt-9 flex flex-col gap-3 sm:flex-row md:mt-12 md:gap-5" aria-label="Primary actions">
              <Button type="button" onClick={() => goToPage("access")} className="relative inline-flex min-h-14 items-center justify-center overflow-hidden rounded-full bg-amber-300 px-7 py-4 text-sm font-semibold uppercase tracking-[0.16em] text-black transition duration-500 hover:bg-amber-200 hover:shadow-[0_0_45px_rgba(251,191,36,0.28)] sm:px-9 md:text-base md:tracking-[0.24em]">
      ...
      <section className="relative z-10 mx-auto max-w-[1500px] px-5 py-8 md:px-8 md:py-10">
        ...
        <div className="mt-16 grid gap-8 lg:mt-20 lg:grid-cols-[0.8fr_1.2fr] lg:items-start">
          <div className="lg:self-start">
            <p className="text-xs uppercase tracking-[0.28em] text-amber-300 md:text-sm md:tracking-[0.35em]">{content.builtAround}</p>
            <h2 className="mt-5 font-serif text-3xl leading-tight text-stone-100 md:mt-6 md:text-7xl">
        ...
        <div className="mt-16 grid gap-8 lg:mt-24 lg:grid-cols-[0.75fr_1.25fr] lg:items-start">
          <div>
            <p className="text-xs uppercase tracking-[0.28em] text-amber-300 md:text-sm md:tracking-[0.35em]">{content.launchChanges.eyebrow}</p>
            <h2 className="mt-5 font-serif text-3xl leading-tight text-stone-100 md:mt-6 md:text-6xl">{content.launchChanges.title}</h2>
            <p className="mt-5 text-base leading-7 text-stone-500 md:mt-6 md:text-lg md:leading-8">{content.launchChanges.text}</p>
          </div>
        ...
        <div className="mt-16 grid gap-8 md:mt-24 lg:grid-cols-[0.85fr_1.15fr] lg:items-start">
          <div className="operator-surface border border-amber-300/20 bg-black/55 p-7 md:p-10 md:backdrop-blur-xl">
            <div className="absolute inset-0 operator-grid opacity-15" aria-hidden="true" />
            <p className="text-xs uppercase tracking-[0.28em] text-amber-300 md:text-sm md:tracking-[0.35em]">{content.subscriptionIntro.eyebrow}</p>
            <h2 className="mt-5 font-serif text-3xl leading-tight text-stone-100 md:mt-6 md:text-6xl">{content.subscriptionIntro.title}</h2>
            <p className="mt-5 text-base leading-7 text-stone-500 md:mt-6 md:text-lg md:leading-8">{content.subscriptionIntro.text}</p>
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="operator-surface mt-16 border border-stone-800 bg-stone-950/45 p-6 md:mt-24 md:p-12">
          <div className="absolute inset-0 operator-grid opacity-15" aria-hidden="true" />
          <p className="text-xs uppercase tracking-[0.28em] text-amber-300 md:text-sm md:tracking-[0.35em]">{content.beforeAfter.eyebrow}</p>
          <h2 className="mt-5 max-w-4xl font-serif text-3xl leading-tight text-stone-100 md:text-6xl">{content.beforeAfter.title}</h2>
