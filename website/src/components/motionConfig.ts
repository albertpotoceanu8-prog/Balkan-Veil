// Reveal timing is tuned per device: phones scroll fast, so effects should land
// early, once, and snappy; desktop keeps the slower cinematic re-reveal.
const prefersMobileReveal = typeof window !== "undefined" && window.matchMedia("(max-width: 767px)").matches;

export const cardMotion = prefersMobileReveal
  ? ({
      // Mobile: animate on mount, not on scroll. whileInView proved unreliable for
      // some cards (staying invisible); a mount fade guarantees they are always shown.
