...
export function DossierPanel({ eyebrow, title, text, index, children, className = "" }: DossierPanelProps) {
  return (
    <article className={`operator-surface relative overflow-hidden border border-stone-800 bg-stone-950/55 p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.035)] transition duration-500 hover:border-amber-300/30 md:p-8 ${className}`}>
      ...
      <div className="pointer-events-none absolute left-5 top-5 flex gap-1" aria-hidden="true">
        <span className="h-1.5 w-1.5 bg-amber-300/45" />
      ...
      <div className="flex items-start justify-between gap-6">
        <div>
          ...
          <h3 className="mt-4 font-serif text-2xl leading-tight text-stone-100 md:text-3xl">{title}</h3>
        </div>
        {index ? <span className="font-serif text-4xl leading-none text-amber-100/15 md:text-5xl">{index}</span> : null}
      </div>
