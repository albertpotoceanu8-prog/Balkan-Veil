          ...
          <div className="border-b border-[#202224] p-6 md:p-8 lg:border-b-0 lg:border-r">
            ...
            <div className="mt-8 flex items-end justify-between gap-6 border-t border-[#252729] pt-6 md:mt-10">
              ...
              <div className="max-w-44 text-right">
                ...
                <p className="font-mono text-[10px] uppercase tracking-[0.26em] text-[#786f5e]">{labels.status}</p>
              </div>
            </div>
          </div>
              ...
              {dossierRows.map(([label, value], index) => (
                <div key={label} className="grid gap-3 py-5 md:grid-cols-[10rem_1fr] md:gap-6">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-mono text-[10px] uppercase tracking-[0.24em] text-[#b98a32]">{label}</span>
                  </div>
                  <p className={`text-base leading-7 ${index === dossierRows.length - 1 ? "font-serif text-xl text-[#c8ad72] md:text-2xl" : "text-[#787873]"}`}>{value}</p>
                </div>
              ))}
            </div>
