...
export function VeilFrame({ children, label, index, className = "" }: VeilFrameProps) {
  return (
    <div className={`veil-frame operator-surface relative overflow-hidden border border-[#202224] bg-[#050302]/88 ${className}`}>
      ...
      {(label || index) ? (
        <div className="relative flex items-center justify-between gap-4 border-b border-[#202224] bg-black/25 px-5 py-3 font-mono text-[10px] uppercase tracking-[0.24em] text-[#6f6654]">
          <span>{label}</span>
