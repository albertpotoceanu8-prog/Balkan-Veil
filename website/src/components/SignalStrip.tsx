...
export function SignalStrip({ items }: SignalStripProps) {
  return (
    <motion.div {...cardMotion} className="operator-surface relative overflow-hidden border border-[#33270f] bg-black/50 p-3 md:p-4 md:backdrop-blur-xl">
      ...
      <div className="relative grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        {items.map((item, index) => (
          <div key={item} className="relative border border-[#241d0c] bg-[#050302]/88 px-4 py-4 transition duration-500 hover:border-[#b99832]/45 md:px-5 md:py-5">
            ...
            <p className="mt-3 font-serif text-lg leading-tight text-[#d8c7a3] md:text-2xl">{item}</p>
          </div>
        ))}
      </div>
    </motion.div>
