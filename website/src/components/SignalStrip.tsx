...
export function SignalStrip({ items }: SignalStripProps) {
  return (
    <motion.div {...cardMotion} className="operator-surface relative overflow-hidden border border-[#3a2410] bg-black/50 p-3 md:p-4 md:backdrop-blur-xl">
      ...
      <div className="relative grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        {items.map((item, index) => (
          <div key={item} className="relative border border-[#2a190b] bg-[#050302]/88 px-4 py-4 transition duration-500 hover:border-[#9a6936]/45 md:px-5 md:py-5">
            ...
            <p className="mt-3 font-serif text-lg leading-tight text-[#d5c0a0] md:text-2xl">{item}</p>
          </div>
        ))}
      </div>
    </motion.div>
